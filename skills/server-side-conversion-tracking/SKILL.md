...
description: 'iOSの制限、広告ブロッカー、Cookie消失があっても購入をFacebook、TikTok、Google、Bingへ正確に報告できるサーバー側コンバージョントラッキングを構成します。コンバージョンの過少報告、プラットフォームの購入数と実注文数の不一致、Conversions API / Events API / offline conversions / CAPI、クリックIDの引き継ぎ（fbclid、ttclid、gclid、msclkid）、トラッキング変更後の広告最適化低下を扱うときに使います。'
---
