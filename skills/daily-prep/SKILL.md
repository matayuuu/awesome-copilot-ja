...
> 「{target date} にはどのような会議がありますか？各会議について、件名、開始時刻、終了時刻、開催者、全参加者とそのメールアドレス、場所、オンライン会議かどうか、私が承諾または辞退したかを含めてください。」

応答が不十分な場合は、追加で問い合わせます。

> 「{target date} の会議のうち、任意参加または仮承諾とされているものはどれですか？また、定期的な会議はどれですか？」
...
| ラベル | 基準 |
|-------|----------|
| `[顧客 · 高]` | 顧客/パートナーの外部ドメインの参加者がいる、または件名が既知の顧客名と一致する |
| `[社内]` | 社内ドメインの参加者だけ |
| `[コミュニティ]` | CoP、コミュニティ、ギルド、学習セッション |
| `[スキル向上]` | トレーニング、ワークショップ、認定、学習 |
| `[任意 · 欠席候補]` | 仮承諾、重要度が低い、または任意参加と分かっている定例会議（例: 「オフィスアワー」「自由参加の質疑応答」） |
| `[個人]` | 個人的な予定、業務外 |
...
| 条件 | マーカー | アクション |
|-----------|--------|--------|
| 15:30 以降 16:00 未満に開始（開催者を問わない） | `⚠️ 時間外` | 辞退を推奨 |
| 16:00 以降に開始し、**自分が開催者ではない** | `⚠️ 時間外` | 辞退を推奨 |
| 16:00 以降に開始し、自分が開催者 | _(フラグなし)_ | 問題なし — 自分で設定した予定 |
| 09:00 より前に開始し、**自分が開催者ではない** | `⚠️ 早朝` | 学習時間を侵食するため辞退を推奨 |
| 09:00 より前に開始し、自分が開催者 | _(フラグなし)_ | 問題なし — 自分で設定した予定 |
| 12:00～13:00 と重なる | `🍽️ 昼休みとの競合` | 予定表の注記に記載 |
...
| 確認項目 | 条件 | フラグ |
|-------|-----------|------|
| **顧客会議の過多** | `[顧客 · 高]` が 2 件を超える | 3 件目以降に「別日への変更を検討」と表示 |
