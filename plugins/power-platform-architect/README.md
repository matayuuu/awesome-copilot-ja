...
A **Microsoft Power Platformのシニアソリューションアーキテクト**として動作するGitHub Copilotプラグインです。ビジネス要件、ユースケースの説明、未整理の会議記録まで入力すると、コンポーネントの推奨と任意のMermaid.js図を備えた、要件に合わせた技術アーキテクチャを作成します。
